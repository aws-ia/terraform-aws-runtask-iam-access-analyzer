...
output "runtask_url" {
  ...
  description = "The Run Tasks URL endpoint, you can use this to configure the Run Task setup in Terraform Cloud"
}

output "runtask_id" {
  value       = tfe_organization_run_task.aws_iam_analyzer.id
  description = "The Run Tasks id configured in Terraform Cloud"
}